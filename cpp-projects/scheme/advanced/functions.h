...
class DefLambda : public Function {
public:
    std::shared_ptr<Object> Apply(const std::shared_ptr<Cell>& cell) override;
};
...
public:
    LambdaFunc(const std::vector<std::shared_ptr<Object>>& args,
...
public:
    ...
    void RegisterFunction(const std::string& func_name) {
        registred_functions[func_name] = std::make_shared<Func>();
    }

    std::unordered_map<std::string, std::shared_ptr<Object>> GetRegisteredFunctions() {
        return registred_functions;
    }
...
protected:
    ...
    ~FunctionFabric() = default;
    std::unordered_map<std::string, std::shared_ptr<Object>> registred_functions;
};
