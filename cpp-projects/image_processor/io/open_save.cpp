...
void SaveFile(const char* path, Image& image) {
    ...
    const int count_paddings = ((4 - (static_cast<int>(image.Width()) * 3) % 4) % 4);
    const int file_size = static_cast<int>(FILE_HEADER_SIZE + INFO_HEADER_SIZE + image.Width() * image.Height() * 3 +
...
void Image::Read(const char* path) {
    ...
    const int count_paddings = ((4 - (static_cast<int>(width_) * 3) % 4) % 4);
